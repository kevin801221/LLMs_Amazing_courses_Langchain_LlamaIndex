...
def get_data_keys():
    return [obj["name"] for obj in data_points if obj["value"] is None]
...
def main():
    ...
    tools = [{
        ...
        "function": {
            "name": "scrape",
            "description": "Scrape a URL for information about a company.",
            "parameters": {
                "type": "object",
                "properties": {
                    "url": {
    ...
    You will NOT stop until ALL missing data points are retrieved.
    You will try as hard as possible to search the web to find information about {company};
    You will search for the following data points: {data_keys_to_search}
    The values in data points are the answers of the data points name, and references are the url you scraping;
    """
    ...
    try:
        ...
        if not response.choices:
            print(colored("No choices in response", "red"))
            return

        message = response.choices[0].message
        if not hasattr(message, 'tool_calls') or not message.tool_calls:
            print(colored("No tool calls in message", "red"))
            return

        for tool_call in message.tool_calls:
            try:
                arguments = json.loads(tool_call.function.arguments)
                print(colored(f"Tool Call Arguments: {arguments}", "cyan"))
                
                url = arguments.get("url")
                if not url:
                    print(colored("No URL in arguments", "red"))
                    continue
                ...
                for chunk in chunk_data(scraped_data, 2000):
                    messages.append(create_tool_message(url, chunk, tool_call.id))
                ...
                print(colored(f"Error processing tool call: {e}", "red"))
                continue

    except Exception as e:
        print(colored(f"Error in main execution: {e}", "red"))
