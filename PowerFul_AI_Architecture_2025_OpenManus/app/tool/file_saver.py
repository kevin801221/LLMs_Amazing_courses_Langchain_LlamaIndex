...
Files will be saved to the D:\\Github_items\\OpenManus\\saved_files directory by default.
"""
    parameters: dict = {
        "type": "object",
        "properties": {
    ...
    DEFAULT_SAVE_DIR: ClassVar[str] = "D:\\Github_items\\OpenManus\\saved_files"
    ...
    async def execute(self, content: str, file_path: str, mode: str = "w") -> str:
        ...
        """
        try:
            # Convert macOS/Unix paths to Windows paths if on Windows
            if platform.system() == "Windows":
                # Check if this is a macOS/Unix style path or just a filename
                if file_path.startswith("/") or not os.path.isabs(file_path):
                    # Extract the filename
                    filename = os.path.basename(file_path)
                    # Use the default save directory
